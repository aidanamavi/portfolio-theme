...
jQuery(document).ready(function() {
  ...
  const homepageDiv = 'page_archive_work'
  // Dynamic settings.
  const ajaxurl = window.location.protocol + '//' + window.location.host + '/wp-admin/admin-ajax.php'
  let visiblePage = jQuery('#content_wrapper :visible').attr('id')
  let pageReferrerUrl = document.referrer || document.location.href
  let isPageLoading = false
  let isSlideLoading = false
  let isThumbnailLoading = false
  let currentProjectType = 'all'
  const isTrackingOn = typeof _paq !== 'undefined'
  ...
  function initiateState() {
    ...
    if (!state) {
      ...
      const viewType = jQuery('#' + visiblePage).data('viewType')
      const postType = jQuery('#' + visiblePage).data('postType')
      const postId = jQuery('#' + visiblePage).data('postId')
      const pageState = { pageUrl, viewType, postType, postId }
  ...
  function addHighlightSlideCursor() {
    jQuery('div.highlight_slides').each(function() {
      const divs = jQuery(this)
      const the_count = divs.children().length
      if (the_count > 1) {
  ...
  function transitionSlide(pageId, oldSlide, newSlide) {
    const newSlideElement = jQuery('div#' + pageId + ' .highlight_slides div[data-slide=' + newSlide + ']')
    const oldSlideElement = jQuery('div#' + pageId + ' .highlight_slides div[data-slide=' + oldSlide + ']')
    ...
    const buttonList = jQuery('#' + pageId + ' .numbers_wrapper .numbers')
    const newButtonImage = buttonList.children('[data-slide=' + newSlide + ']').children('img.off')
    const oldButtonImage = buttonList.children('[data-slide=' + oldSlide + ']').children('img.off')
  ...
  function loadPage(pageUrl, viewType, postType, postId, isPushHistory) {
    ...
    const pageDiv = getPageDiv(viewType, postType, postId)
    ...
    if (isEmpty(jQuery('#' + pageDiv))) {
      jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: { action: 'getAjaxData', viewType, postType, postId, token: window.nonce },
    ...
    if (isPushHistory) {
      const pageTitle = jQuery('#' + pageDiv).data('pageTitle')
      updateBrowserHistory({ pageUrl, viewType, postType, postId }, pageTitle, pageUrl)
    }
  } // loadpage()
  ...
  function adjustSlideHeight() {
    ...
    if (postType === 'work' || postType === 'about') {
      ...
      const numbersWrapperHeight = 70 // 34.6px
      let slideHeightImg = jQuery('.slide:visible img.highlight').height() // 529px
      if (!parseInt(slideHeightImg)) { slideHeightImg = 0 }
      const slideHeightText = jQuery('.slide:visible .highlight_text').height() // 529px
      const slideHeight = jQuery('.slide:visible').height()
      const slideHeight2 = slideHeightImg + slideHeightText
      ...
      const cssHeight = numbersWrapperHeight + slideHeight2
  ...
  function updateCategory(pageDiv) {
    ...
    const postType = jQuery('#' + pageDiv).data('postType')
    const categoryId = jQuery('#' + pageDiv).data('categoryId')
    // console.log('postType(' + postType + ')');
  ...
  function updateBreadcrumb(pageDiv) {
    ...
    const pageTitle = jQuery('#' + pageDiv).data('pageTitle')
    const viewType = jQuery('#' + pageDiv).data('viewType')
    const postType = jQuery('#' + pageDiv).data('postType')
    const pageSeperator = ' › '
    let newSiteTitle
  ...
  jQuery(document).on('click', 'a', function(event) {
    ...
    if (linkType === 'headerNavigation') {
      ...
      internalLink()
      let projectType = link.data('projectType')
      ...
      const categoryId = link.data('categoryId')
      const theId = postId || categoryId
  ...
  jQuery(document).on('click', '#content_wrapper div .numbers_wrapper .numbers div', function() {
    ...
    const pageId = button.parent().parent().parent().attr('id')
    const newSlide = button.data('slide')
    const oldSlide = jQuery('#' + pageId + ' .highlight_slides > div:visible').data('slide')
  ...
  jQuery(document).on('click', '#content_wrapper div .highlight_slides div img.highlight', function() {
    const slide = jQuery(this).parent()
    const pageId = slide.parent().parent().attr('id')
    const oldSlide = slide.data('slide')
    const totalDivs = slide.parent().children().length
    if (totalDivs > 1 && !isSlideLoading) {
      isSlideLoading = true
      let nextDiv = slide.next('div')
      let newSlide
      if (nextDiv.length === 0) {
  ...
  /**
	 ...
	 * uses isPageLoading.
	 */
  let categoryId, offsetPosts, areAllPostsLoaded, windowHeight, scrollPosition
